// Admin panel - GitHub Pages friendly
...
async function login(){
  ...
  if (!password) { loginFeedback.innerHTML = `<div class="alert err">Ingresá la contraseña.</div>`; return; }
  try {
    ...
    setLoggedIn(true); await refreshDoctors();
  } catch (e) { loginFeedback.innerHTML = `<div class="alert err">${e.message}</div>`; }
}
...
addDoctorBtn.addEventListener('click', async ()=>{
  ...
  if (!name || !spec){ addDoctorFeedback.innerHTML = `<div class="alert err">Completá nombre y especialidad.</div>`; return; }
  try {
    ...
    refreshDoctors();
  } catch (e) { addDoctorFeedback.innerHTML = `<div class="alert err">${e.message}</div>`; }
});

function generateTimes(from, to, stepMin){
  const [fh, fm] = from.split(':').map(Number);
  const [th, tm] = to.split(':').map(Number);
  let start = fh*60 + fm; const end = th*60 + tm; const out = [];
  while (start < end){ const h = String(Math.floor(start/60)).padStart(2,'0'); const m = String(start%60).padStart(2,'0'); out.push(`${h}:${m}`); start += stepMin; }
  return out;
}

let generated = [];
generateSlotsBtn.addEventListener('click', ()=>{
  generatedSlots.innerHTML = ''; generated = [];
  const d = dateAdmin.value; const from = fromTime.value; const to = toTime.value;
  const step = Math.max(5, parseInt(intervalMin.value || '30', 10));
  if (!d || !from || !to) { generatedSlots.innerHTML = `<div class="alert err">Completá fecha, desde y hasta.</div>`; return; }
  const times = generateTimes(from, to, step);
  if (!times.length){ generatedSlots.innerHTML = `<div class="alert">No hay horarios en ese rango.</div>`; return; }
  times.forEach(t => {
    const btn = document.createElement('button'); btn.className = 'time-btn'; btn.textContent = t + ' hs';
    btn.addEventListener('click', ()=>{
      if (btn.classList.contains('selected')){ btn.classList.remove('selected'); generated = generated.filter(x=>x.time!==t); }
      else { btn.classList.add('selected'); generated.push({ date: d, time: t }); }
...
saveSlotsBtn.addEventListener('click', async ()=>{
  ...
  try {
    ...
    generatedSlots.innerHTML = ''; generated = [];
  } catch (e) { saveSlotsFeedback.innerHTML = `<div class="alert err">${e.message}</div>`; }
});

loadAvailBtn.addEventListener('click', async ()=>{
  const did = doctorFilterAvail.value; const d = dateFilterAvail.value;
  const url = new URL(`${API_BASE}/api/admin/availability`);
  if (did) url.searchParams.set('doctor_id', did);
  if (d) url.searchParams.set('date', d);
  const res = await fetch(url, { headers: { 'x-admin-token': token } });
  const data = await res.json();
  if (!Array.isArray(data) || !data.length){ availList.innerHTML = `<div class="alert">Sin resultados.</div>`; return; }
  const list = document.createElement('div'); data.forEach(item => {
    ...
    row.innerHTML = `<strong>${item.date}</strong> — ${item.time} hs · ${item.doctor_name} (${item.specialty})
      <div style="float:right; display:inline-flex; gap:8px;"><button class="ghost" data-id="${item.id}">Eliminar</button></div>`;
    list.appendChild(row);
  });
  availList.innerHTML = ''; availList.appendChild(list);
  [...availList.querySelectorAll('button.ghost')].forEach(btn => {
    btn.addEventListener('click', async ()=>{
      const id = btn.getAttribute('data-id');
      if (!confirm('¿Eliminar este horario?')) return;
      const res2 = await fetch(`${API_BASE}/api/admin/availability/` + id, { method: 'DELETE', headers: { 'x-admin-token': token } });
      const j = await res2.json();
      if (res2.ok){ btn.closest('.card').remove(); } else { alert(j.error || 'Error'); }
    });
  });
});

loadBookingsBtn.addEventListener('click', async ()=>{
  const did = doctorFilterBook.value; const d = dateFilterBook.value;
  const url = new URL(`${API_BASE}/api/admin/bookings`);
  if (did) url.searchParams.set('doctor_id', did);
  if (d) url.searchParams.set('date', d);
  const res = await fetch(url, { headers: { 'x-admin-token': token } });
  const data = await res.json();
  if (!Array.isArray(data) || !data.length){ bookingList.innerHTML = `<div class="alert">Sin resultados.</div>`; return; }
  const list = document.createElement('div'); data.forEach(b => {
  ...
  bookingList.innerHTML = ''; bookingList.appendChild(list);
  [...bookingList.querySelectorAll('button.ghost')].forEach(btn => {
    btn.addEventListener('click', async ()=>{
      const id = btn.getAttribute('data-id');
      if (!confirm('¿Cancelar este turno? Se repondrá la disponibilidad.')) return;
      const res2 = await fetch(`${API_BASE}/api/admin/bookings/` + id, { method: 'DELETE', headers: { 'x-admin-token': token } });
      const j = await res2.json();
      if (res2.ok){ btn.closest('.card').remove(); } else { alert(j.error || 'Error'); }
    });
  });
